...
class Edrone():
    ...
    def __init__(self):
        ...
        self.setpoints =[[18.9999864489,71.9999430161,11.44099749139,0],[18.9999864489,71.9999430161,8.4,0],[19.0000135529,71.9999430161,11.44099749139,0],[19.0000135529,71.9999430161,8.4,0],[19.0000000009,71.9999572611,11.44099749139,0],[19.0000000009,71.9999572611,8.4,0],[19.0,72.0,8.44+3,0]] # Global setpoints
        ...
        if(elapsed_time>=0.001): # Sample time
            ...
            if(index_no!=len(self.setpoints) and index_no<len(self.setpoints)):
                ...
                # If the current coordinates is within the bounded range then lat/longt/alt is set to 1,else it is set to 0
                    ...
                    if(((self.setpoints[index_no][0]-0.000005500)<self.current_coord[0]) and ((self.setpoints[index_no][0]+0.000005500)>self.current_coord[0])):
                        lat=1
                    else:
                        lat=0 
           
                    if(((self.setpoints[index_no][1]-0.000005500)<self.current_coord[1]) and ((self.setpoints[index_no][1]+0.000005500)>self.current_coord[1])):
                        longt=1
                    else:
                        longt=0  
                ...
                if(lat+longt+alt==3):
                    ...
                    if(index_no<=(len(self.setpoints)-1) and self.search!=1):
                        ...
                        self.path_dist=(((self.setpoints[index_no][0]-self.setpoints[index_no-1][0])*110692.0702932625)**2+((self.setpoints[index_no][1]-self.setpoints[index_no-1][1])*105292.0089353767)**2)**0.5 # Calculating distance between current setpoint and destination setpoint in metres       
                        
                        if(self.path_dist>30): # If this distance is more than 30 metres then make a path to follow
                            self.path_planner(index_no)          
        ...
        if(self.change==1): # Value of change is 1 when it is beginning,required local setpoint is reached or when during escape drone encounters an obstacle
            ...
            if(self.path=="X"):           
                if(self.back==1 and self.sign_x!=1): 
                    ...
                    longi=self.current_coord[1]+(4.5/105292.0089353767) # For moving along obstacle
                    self.obstacle="BACK"
                    self.setpoints_local.append([lati,longi,self.current_coord[2]])
                    

                if(self.front==1 and self.sign_x!=-1): 
                    lati=self.current_coord[0] - ((4.5-self.obstacle_dist_x)/110692.0702932625)
                    longi=self.current_coord[1]+(4.5/105292.0089353767)    
                    self.obstacle="FRONT"  
                    self.setpoints_local.append([lati,longi,self.current_coord[2]])      
            ...
            if(self.escape_change==1 and self.count==1):
                ...
                if(self.path=="X"):
                    ...
                    if(self.obstacle=="FRONT" or self.obstacle=="BACK"):
                        lati=self.current_coord[0]+(6/110692.0702932625)*self.sign_x
                    ...
                    if(self.obstacle=="LEFT"):
                        lati=self.current_coord[0]+(0.5/110692.0702932625)*self.sign_x
...
if __name__ == '__main__':
    ...
    csv_list=[] # To store data from csv file
    with open("/home/utkarsh/catkin_ws/src/vitarana_drone/scripts/manifest.csv","r") as csv_file:
    ...
    index=0            # Variable for accessing data inside csv_list
    j=0                # Variable for accessing and manipulating data inside setpoints list of Edrone class
    while(index!=len(csv_list)):
        j=j+2
        e_drone.setpoints.insert(j,[e_drone.setpoints[j-1][0],e_drone.setpoints[j-1][1],float(csv_list[index][3])+16.75,1])
        j=j+1
        e_drone.setpoints.insert(j,[float(csv_list[index][1]),float(csv_list[index][2]),float(csv_list[index][3])+16.75,1])
        e_drone.target_setpoints.append([float(csv_list[index][1]),float(csv_list[index][2]),float(csv_list[index][3])+16.75,1])
        j=j+1
        index=index+1
